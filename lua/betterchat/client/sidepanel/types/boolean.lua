function chatBox.sidePanel.renderSettingFuncs.boolean( sPanel, panel, data, y, w, h, setting )
    ...
    function checkBox:Paint( w, h )
        ...
        draw.RoundedBox( 0, 0, 0, w, h, chatBox.defines.gray( c, 255 ) )
        draw.RoundedBox( 0, 2, 2, w - 4, h - 4, chatBox.defines.gray( c - 40, 255 ) )

        if ticked then
            if disabled then
                draw.RoundedBox( 0, 3, 3, w - 6, h - 6, ch.setA( chatBox.defines.theme.sidePanelCheckBox, 50 ) )
            else
                draw.RoundedBox( 0, 3, 3, w - 6, h - 6, chatBox.defines.theme.sidePanelCheckBox )
            end
        end
    end
